...
mobile tools for onnx or torchscript
...
if __name__ == '__main__':
    ...
    t.save_to_part(tmp_dir='./output') # ./output x.npy, y.npy and ./x.npy

    # load
    t = IOTensor(tensor_path='./x.npy')
    print(t.shape) # print shape
    print(t.range) # print data range
...
### OnnxModel module
```python
from mtools import OnnxModel, print_tensor_shape
import numpy as np

if __name__ == '__main__':
    model = OnnxModel('/home/dingzhifeng/iwork/aigc-mobile/models/stable_diffusion/anything-v3.0/qualcomm-256-256/unet/unet.down.onnx')
    sample = np.random.randn(2, 4, 32, 32).astype(np.float32) 
    timestep = np.random.randn(2, 320).astype(np.float32)
    encoder_hidden_states = np.random.randn(2, 77, 768).astype(np.float32)

    result = model([sample, timestep, encoder_hidden_states])
...
### Calc Diff
```python
from mtools import diff
import numpy as np

if __name__ == '__main__':
    x = np.random.randn(2, 4, 32, 32)
    y = np.random.randn(2, 4, 32, 32)

    jdict = diff(x, y, eps=1e-3, bins=10)
```
