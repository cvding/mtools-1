...
def print_tensor_shape(inputs, name):
    if isinstance(inputs, (torch.Tensor, np.ndarray)):
        print(name, ' ', list(inputs.shape))
    elif isinstance(inputs, dict):
        for k, v in inputs.items():
            print_tensor_shape(v, name+'->' + k)
    elif isinstance(inputs, (tuple, list)):
        for idx, v in enumerate(inputs):
            print_tensor_shape(v, name + '_%d' % idx)
    else:
        print(name, inputs, "(value)")
...
def vsummary(model: nn.Module,
    ...
    **kwargs: Any,):
    ...
    flops, params, result = profile(model, input_data, verbose=False)
